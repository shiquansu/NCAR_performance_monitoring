...
sub convert_mem {
	...
	#no value change in convertion, ganglia reports in unit of KB
...
my $sts = getopt('f:s:e:Z:a:h:d:', \%args);

if (!defined($sts) || $#ARGV != 0) {
    print "Usage: ganglia2pcp [-s start] [-e end] [-f output file name] [-d output file dir ] [-Z timezone] [-a adminnodename] [-h hostname] input_dir \n";
    exit(1);
}
